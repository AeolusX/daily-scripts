...
download_url=http://repo-ops.com
...
get_soft(){
    #软件下载
    if [ ! -e redis-5.0.3.tar.gz ]; then
        echo "开始下载redis-2.8.23.tar"
        wget ${download_url}/soft/redis/redis-5.0.3.tar.gz
        echo "软件下载成功，开始解压配置"
    fi
}
...
Install(){
    ...
    echo "开始解压软件."
    if [ ! -e redis-5.0.3.tar.gz ]; then
        echo -e "\033[31mError: redis-5.0.3.tar.gz下载失败，请检测网络\033[0m"
        exit 0
    fi
...
config(){
    ...
    wget -P ./redis_tmp/ ${download_url}/soft/config/redis/redis-5.0.conf
    if [ -f ./redis_tmp/redis-5.0.conf ];then
        sed -i "s/{Port}/${2}/g" ./redis_tmp/redis-5.0.conf
        cp -rf ./redis_tmp/redis-5.0.conf ${1}/redis.conf
        chown -R root.root ${1}
