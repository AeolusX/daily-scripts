...
download_url=http://repo-ops.soft.com/soft/mongodb
...
get_soft(){
    ...
    if [ ! -e mongodb-linux-x86_64-2.6.11.tar ]; then
        ...
        wget ${download_url}/mongodb-linux-x86_64-2.6.11.tar
        echo "软件下载成功，开始解压配置"
        Install
    fi
}
...
edit_start_file(){
    ...
    wget -P ./mongo_tmp/ ${download_url}/init/mongo_template
...
if [ $# -gt 0 ]; then
    if [ ${1} -ne 27017 -a ${1} -ne 27018 -a ${1} -ne 27018 -a ${1} -ne 28017 -a ${1} -ne 28018 -a ${1} -ne 28019 -a ${1} -ne 29017 -a ${1} -ne 29018 -a ${1} -ne 29019];then
        echo -e $usage
        exit 0
    else
        mongo_port=${1}
