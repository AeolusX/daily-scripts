...
get_soft(){
    #软件下载
    if [ ! -e mysql-5.5.53-linux2.6-x86_64.tar.gz ]; then
        echo "开始下载mysql-5.5.53-linux2.6-x86_64.tar.gz"
        wget ${download_url}/mysql-5.5.53-linux2.6-x86_64.tar.gz
        echo "软件下载成功，开始解压配置"
    fi
}
...
Insall(){
    ...
    echo "解压已完成。"
    mv mysql-5.5.53-linux2.6-x86_64/* ${1}
    rm -rf mysql-5.5.53-linux2.6-x86_64
    echo "目录移动已完成##########################"
...
import_config(){
    ...
    echo "开始导入my.cnf############################"
    wget -P ./mysql55_tmp/ ${download_url}/init/mysql55/my_templet.cnf
    wget -P ./mysql55_tmp/ ${download_url}/init/mysql55/mysqld_templet
