...
TIME=`date --date=@$(($(date +%s)-60*24*60)) +%s`
...
# BINARY Details
################################################
MONGODUMP_BIN=/home/mongo/bin/mongodump
...
# DB backup
#########################################
# dump increment data (1 day ago)
$MONGODUMP_BIN --directoryperdb -u $MONGO_USER -h $MONGO_HOST -d $MONGO_DB --oplog -s $TIME -o $DUMP_FOLDER -p "$MONGO_PWD" >/dev/null
if [ $? -ne 0 ]; then
...
/usr/bin/find $DUMP_FOLDER -name "*.bson" -exec gzip {} \;
echo "Dump done."; echo
